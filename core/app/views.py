...
class UserListCreateView(APIView):
    """
    GET  /api/users/   – read-heavy сценарий
    ...
    def get(self, request):
        ...
        users = async_to_sync(self._get_users)()
    ...
    def post(self, request):
        ...
        serializer = UserSerializer(data=request.data)
        if serializer.is_valid():
            async_to_sync(self._save_user)(serializer)
...
class OrderCreateView(APIView):
    """
    POST /api/orders/ – write-heavy сценарий
    """
    ...
    def post(self, request):
        ...
        try:
            order = async_to_sync(self._create_order)(user_id, price)
        except User.DoesNotExist:
            logger.warning(
                f"POST /api/orders/ | invalid user_id={user_id}"
            )
            return Response(
