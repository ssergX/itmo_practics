...
| **Validate** | Установка зависимостей (Poetry) + проверка импортов + наличие Dockerfile |

Валидация выполняется параллельно для всех фреймворков (матричная стратегия). Docker smoke-тесты запускаются локально (`bench_all.py`).
